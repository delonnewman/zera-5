...
JSGLOBAL.wonderscript.compiler = function () {
    ...
    function findNamespaceVar(s) {
        if (s.indexOf("/") !== -1) {
            var parts = s.split("/");
            if (parts.length !== 2)
                throw new Error("A symbol should only have 2 parts");
        ...
        if (!isTaggedValue(form)) {
            ...
            if (SPECIAL_FORMS[name]) {
                ...
                return [
                    DOT_SYM,
                    form.slice(1)[0],
                    [name.slice(1)].concat(form.slice(2)),
                ];
    ...
    function emitMap(m, env) {
        return str(
            "({",
            map(emitMapEntry(env), Object.entries(m)).join(", "),
            "})"
        );
    }
    ...
    function emitArrayMutation(form, env) {
        if (form.length !== 4)
            throw new Error(
                str("Wrong number of arguments expected 3, got ", form.length)
            );
    ...
    function emitArrayLength(form, env) {
        if (form.length !== 2)
            throw new Error(
                str("Wrong number of arguments expected 1, got ", form.length)
            );
    ...
    function emitQuotedValue(val) {
        if (isString(val)) {
            ...
            return str(
                "({",
                map(
                    (xs) => str(xs[0], ":", emitQuotedValue(xs[1])),
                    Object.entries(val)
                ).join(", "),
                "})"
            );
    ...
    function emitThrownException(form, env) {
        if (form.length !== 2)
            throw new Error(
                str("Wrong number of arguments should have 2, got", form.length)
            );
    ...
    function emitCond(form, env) {
        ...
        for (i = 0; i < exprs.length; ++i) {
            ...
            if (exprs[i][0] === "else") {
                buff.push(
                    str("else { ", emitTailPosition(exprs[i][1], env), " }")
                );
            } else {
                ...
                buff.push(
                    str(
                        ...
                        x,
                        " !== false){ ",
                        emitTailPosition(exprs[i][1], env),
                        " }"
                    )
                );
            }
    ...
    function emitLoop(form, env_) {
        ...
        if (form.length < 3)
            throw new Error(
                "A loop expression should have at least 3 elements"
            );
    ...
    function emitAssignment(form, env) {
        if (form.length !== 3)
            throw new Error("set! should have 3 and only 3 elements");
        return str(emit(form[1], env), " = ", emit(form[2], env));
    }
    ...
    function emitFunc(form, env_, opts) {
        ...
        if (form.length < 2)
            throw new Error(
                "a function requires at least an arguments list and a body"
            );
        else {
            if (!isArray(args))
                throw new Error("an arguments list is required");
    ...
    function prStr(x) {
        if (x == null) return NIL_SYM;
            ...
            return str(
                "{",
                map(
                    (entry) => str(prStr(entry[0]), " ", prStr(entry[1])),
                    Object.entries(x)
                ).join(" "),
                "}"
            );
    ...
    define(
        TOP,
        "js",
        IS_NODE ? createNs("global", global) : createNs("window", window)
    );
