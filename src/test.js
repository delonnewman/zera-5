...
zera.test = (function () {
    ...
    ok(
        eq(xs, list(9, 8, 7, 6, 5, 4, 3, 2, 1, 0)),
        "(loop (xs nil, i 0) (cond (= i 10) xs else (recur (cons i xs) (+ i 1))))"
    );
    ...
    ok(
        eq(list("quote", "test"), z.readString(":test").next()),
        "reader: :test"
    );
    ok(
        eq(list("quote", "test"), z.readString("'test").next()),
    ...
    ok(
        eq(list(1, 2, 3, 4), z.readString("(1 2 3 4)").next()),
        "reader: (1 2 3 4)"
    );
    ...
    ok(
        eq(
            list("fn", list("x"), list("fn", list(), "x")),
            z.readString("(fn (x) (fn () x))").next()
        ),
        "reader: (fn (x) (fn () x))"
    );
